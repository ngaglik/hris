...
export default defineComponent({
  setup() {
    ...
    const fetchData = async (page = 1) => {
      loading.value = true
      const response = await apiFetch(Config.UrlBackend+`/api/position?page=${page}&pageSize=${pageSize.value}&inputSearch=${inputSearch.value}`, {
        method: "GET"
      });
